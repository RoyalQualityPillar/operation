...
@Component({
  selector: 'app-create-update-column-perfm-test-reg',
  standalone: false,
  templateUrl: './create-update-column-perfm-test-reg.component.html',
  styleUrl: './create-update-column-perfm-test-reg.component.scss'
})
export class CreateUpdateColumnPerfmTestRegComponent implements OnInit {
  isReadOnly = true;
  isUpdate = false;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<CreateUpdateColumnPerfmTestRegComponent>,
  ...
  onloadDropDown() {
    this.isLoading = true;
    this.columnPerfmTestRegService.getDropDownList().subscribe((data: any) => {
  ...
  onLoadFormValue() {
    ...
    this.apiService
      .sendRequest(apiEndPoints.cptrMasterLoadUpdatePage, 'POST', params)
      .subscribe((data: any) => {
        if (data.data == null) {
          this.isLoading = false;
 ...
 async onSaveConfirmation() {
  ...
  openBusinessUnitCodeLOV2() {
    ...
    const dialogRef = this.dialog.open(LovDialogComponent, {
      ...
      width: '600px',
      data: {
        dialogTitle: 'ColumnPerfmTestReg Master',
        dialogColumns: this.displayedColumns,
