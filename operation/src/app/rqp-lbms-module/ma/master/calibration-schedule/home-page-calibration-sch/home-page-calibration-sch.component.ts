...
import { CreateUpdateCalibrationSchComponent } from '../create-update-calibration-sch/create-update-calibration-sch.component';
import { Router } from '@angular/router';

@Component({
  selector: 'app-home-page-calibration-sch',
  standalone: false,
  templateUrl: './home-page-calibration-sch.component.html',
  styleUrl: './home-page-calibration-sch.component.scss'
})
export class HomePageCalibrationSchComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.allcalibrationschMasterTabledataUrl =
      apiEndPoints.allcalibrationschMasterTabledata;
    this.pageIndex = 0;
    let size = GlobalConstants.size;
    let pageIndex = this.pageIndex;
    let unitCode = this.cookieService.get('buCode');
    this.params = { pageIndex, size, unitCode };
    this.filterApiUrl = apiEndPoints.calibrationschMasterUserProfileFilterData;
    this.activecalibrationschMasterTabledataUrl =
      apiEndPoints.activecalibrationschMasterTabledata;
    this.params = { pageIndex, size, unitCode };
  ...
  async loadRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('apiUrl', this.allcalibrationschMasterTabledataUrl);
      compRef.setInput('tableTitle', 'All Calibration Schedule');
      ...
      compRef.setInput('downloadFileName', ' Calibration Schedule');
  ...
  async loadActiveRoleMasterTableFilter() {
    try {
      ...
      compRef.setInput('columnConfig', this.columnConfig);
      compRef.setInput('filterOptions', this.filterOptions);
      compRef.setInput('apiUrl', this.activecalibrationschMasterTabledataUrl);
      compRef.setInput('tableTitle', 'Active Calibration Schedule');
  ...
  async onActiveSelectAuditRow() {
    let tableData = [
      ...
      { labelName: 'Schedule No', value: this.selectedRow.uc0001 },
      { labelName: 'Module No', value: this.selectedRow.ff0001 },
      { labelName: 'Data Sheet No', value: this.selectedRow.ff0002 },
      { labelName: 'Frequency Code', value: this.selectedRow.ff0003 },
      { labelName: 'Start Date ', value: this.selectedRow.ff0004 },
      { labelName: 'Schedule Count Type', value: this.selectedRow.ff0005 },
      { labelName: 'Schedule Count', value: this.selectedRow.ff0006 },
      { labelName: 'Frequency Type', value: this.selectedRow.ff0007 },
    ...
    if (this.selectedRow.length == 0) {
      ...
      const dialogRef = this.dialog.open(component, {
        minWidth: '80%',
        data: { tableData: tableData, pageTitle: 'Calibration Schedule' },
      });
      dialogRef.afterClosed().subscribe((result) => { });
    }
  }
  ...
  async structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Schedule No', value: item.uc0001 },
          { labelName: 'Module No', value: item.ff0001 },
          { labelName: 'Data Sheet No', value: item.ff0002 },
          { labelName: 'Frequency Code', value: item.ff0003 },
          { labelName: 'Start Date', value: item.ff0004 },
          { labelName: 'Schedule Count Type ', value: item.ff0005 },
          { labelName: 'Schedule Count', value: item.ff0006 },
          { labelName: 'Frequency Type', value: item.ff0007 },
    ...
    const dialogRef = this.dialog.open(component, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: ' Calibration Schedule' },
    });
    dialogRef.afterClosed().subscribe((result) => { });
  }
  columnConfig = {
    ...
    uc0001: 'Schedule No',
    ff0001: 'Module Code',
    ff0002: 'Data Sheet No',
    ff0003: 'Frequency Code',
    status: 'Status',
    version: 'Version',
    createdon: 'CreatedOn',
    createdby: 'CreatedBy',
  ...
  tableTitle: string = 'All  Calibration Schedule';
  allButtonConfig = [
