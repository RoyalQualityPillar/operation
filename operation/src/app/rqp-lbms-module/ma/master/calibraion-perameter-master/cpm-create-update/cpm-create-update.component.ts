...
@Component({
  selector: 'app-cpm-create-update',
  standalone: false,
  templateUrl: './cpm-create-update.component.html',
  styleUrl: './cpm-create-update.component.scss'
})
export class CpmCreateUpdateComponent   implements OnInit {
  isReadOnly = true;
  isUpdate = false;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<CpmCreateUpdateComponent>,
  ...
  onLoadFormValue() {
    ...
    this.apiService
      .sendRequest(apiEndPoints.CpmLoadUpdatePage, 'POST', params)
      .subscribe((data: any) => {
        if (data.data == null) {
          this.isLoading = false;
