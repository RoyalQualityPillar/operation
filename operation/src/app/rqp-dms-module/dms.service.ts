...
//import { SUBMITMODEL } from './models/urs-models';
...
export class DmsService {
  ...
  onSOPAttachments(referenceAttachments:any[],body:any){
...
formData.append('attachmentDtoList', jsonBlob, 'data.json');
console.log(formData);

let createUserURL = this.API_URL + 'gm/attachments';
const httpOptions = {
  headers: new HttpHeaders({
    Authorization: 'Bearer' + token,
  }),
...
allstagesrecords(unitCode: string,moduleCode: string,pageIndex: number,size: number): Observable<any> {
  return this.http.get(
    this.API_URL +
    `gm/gmur-record/todo-all-stages?FF0003=${unitCode}` +
    `&FF0005=${moduleCode}` +
    `&pageIndex=${pageIndex}` +
    `&size=${size}`
  );
