...
export class InspectionTypeHomePageComponent implements OnInit, AfterViewInit {
  ...
  allInspectionTableDataUrl: any;
  activeInspectionTableDataUrl: any;
  filterApiUrl: any;
  params: any;
  HttpMethod = 'POST';
  ...
  ngOnInit(): void {
    this.allInspectionTableDataUrl = apiEndPoints.allInspectionTabledata;
    this.pageIndex = 0;
    ...
    this.activeInspectionTableDataUrl = apiEndPoints.activeInspectionTabledata;
    this.params = { pageIndex, size, unitCode };
  }
  ...
  onSearchAllAuditTrail() {
    ...
    if (this.selectedAllRow.length == 0) {
      ...
      const params = { UC0001 };
      this.apiService
        .sendRequest(apiEndPoints.inspectionAllAuditTrail, 'GET', params)
        .subscribe((data: any) => {
  ...
  structureResponse(apiResponse: any) {
    const rows = apiResponse.map((item) => {
      return {
        fields: [
          ...
          { labelName: 'Inspection Type No', value: this.selectedRow.uc0001 },
          { labelName: 'Inspection Type Name', value: this.selectedRow.ff0001 },
    ...
    const dialogRef = this.dialog.open(CommonAllAuditTrailComponent, {
      minWidth: '80%',
      data: { tableData: rows, pageTitle: 'Inspection Type Master' },
    });
    dialogRef.afterClosed().subscribe((result) => {});
  }
  columnConfig = {
    action: 'Action',
    uc0001: 'Inspection Type No',
